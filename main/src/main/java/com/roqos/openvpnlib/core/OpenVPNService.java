...
import static com.roqos.openvpnlib.core.ConnectionStatus.LEVEL_CONNECTED;
import static com.roqos.openvpnlib.core.ConnectionStatus.LEVEL_WAITING_FOR_USER_INPUT;
...
//            addVpnActionsToNotification(nbuilder);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if(enableLogWindow || status == LEVEL_CONNECTED) {
                mNotificationManager.notify(notificationId, notification);
                startForeground(notificationId, notification);
            }
        }
